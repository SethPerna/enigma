  ...
  def encrypt(message, key = key_generator, date = today_date)
    ...
    message_array.each_with_index do |char, i|
      ...
      if !@char_set.include?(char) == true
        ...
        encrypt_1 = @char_set.rotate((a_shift + offset_2[0].to_i))[@char_set.index(char)]
          encrypted_message << encrypt_1
      elsif i % 4 == 1
        encrypt_2 = @char_set.rotate((b_shift + offset_2[1].to_i))[@char_set.index(char)]
          encrypted_message << encrypt_2
      elsif i % 4 == 2
        encrypt_3 = @char_set.rotate((c_shift + offset_2[2].to_i))[@char_set.index(char)]
          encrypted_message << encrypt_3
      elsif i % 4 == 3
        encrypt_4 = @char_set.rotate((d_shift + offset_2[3].to_i))[@char_set.index(char)]
